...
## Own instructions and plain calls (probed 2026-09-05, codex-cli 0.153.4)

- `-c model_instructions_file=PATH` replaces the whole `instructions` field of every request with the file's content (verified through a dumping proxy: nothing of Codex's 17 KB prompt remains). The model still gets Codex's tools and edits files fine under a 268-byte prompt. `-c developer_instructions=TEXT` is prepended to the developer message instead. Forward slashes in the path avoid TOML escaping.
- Config keys that exist in this build (from the binary): `include_permissions_instructions`, `include_apps_instructions`, `include_collaboration_mode_instructions`, `include_environment_context`, `skills.include_instructions`, `features.skills`, `features.plugins`, `features.collaboration_modes`, `tools.view_image`, `compact_prompt`, `personality`. `skills.bundled` is a table, not a bool; `mcp_servers.cua_repl.enabled` is rejected ("invalid transport") because `cua_repl` is built in. A rejected key makes the app-server exit immediately; codexctl now reports its stderr instead of "timeout waiting for initialize".
- What each switch removed, measured on the same prompt: lean set 48 KB → + own instructions 15 KB → + `features.plugins=false` (also drops the `cua_repl` tool), `features.collaboration_modes=false`, `include_permissions_instructions=false`, `include_environment_context=false`, `skills.include_instructions=false` (the skills catalog stays without this; `features.skills=false` alone does not remove it) → 7.4 KB, 1064 input tokens on Gemini. `tools.view_image=false` did not remove `view_image` and `request_user_input` stays; the remaining 5.2 KB is tool schemas (`exec_command` 2.6 KB, `write_stdin`, `request_user_input`, `view_image`, `web_search`) that no key removes.
- All of llmctl's model ids answer on OpenRouter's Responses path: muse-spark-1.3, gemini-3.8-flash, claude-sonnet-5, deepseek-v4-flash, qwen3.7-flash, glm-5.3-flash, mimo-v2.5, kimi-k3, gpt-5.4-mini, claude-haiku-4-5, gemini-3.5-flash-lite (tiny direct calls, 0.7-11 s). `codexctl ask/compare/map` use these as plain calls: `ask flash` 2.1 s wall (about 1 s of that is spawning the app-server and starting the ephemeral thread), `compare --models prose --judge default` 22 s for 3 variants plus judge, `map` 3 chunks on deepseek 11 s. Cost is computed from OpenRouter's model catalog (cached a day in `~/.codexctl/openrouter-models.json`) and logged to `~/.codexctl/usage.jsonl`.
- Node on Windows: calling `process.exit()` right after killing the app-server child can hit a libuv assertion (`!(handle->flags & UV_HANDLE_CLOSING)`, async.c) after the output was already written. Plain commands set `process.exitCode` and let the loop drain instead.
